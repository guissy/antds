...
import {type Component, type JSX, createEffect, createSignal, createContext, createMemo, useContext, children as Children, Accessor, splitProps} from "solid-js";
...
function useControl<T>(valuePropName: string, defaultValue: T): [Accessor<T>, any] {
...
const LabelItem: Component<{
  ...
  const [_, rest] = splitProps(props, ['title', 'children'])
  const childNode = Children(() => props.children)() as HTMLInputElement;
  if (!childNode) return '';
  const type = childNode.nodeName.toLowerCase();
  spread(childNode, rest);
  createEffect(() => {
    console.log('type',type);
    if (type === 'input' || type === 'select') {
      childNode.value = rest.value();
    }
  })
  if (type === 'input' && childNode.type === 'checkbox') {
    return (
      <label style={style}>
